...
function formatName(nameObject){
...
let users = parsedData.result 
let currentIndex = 0;
//let currentUser = users[0];
let currantUser = users[currentIndex]

function update(){
  
  let name = document.querySelector(".name");
  name.style.color = 'red';
  name.innerText = formatName(currentUser.name)
  let phone = document.querySelector(".phone");
  phone.innerText = formatPhone(currentUser.phone)
  let email = document.querySelector(".email");
  email.innerText = (currentUser.email)
  let image = document.querySelector(".profilepic");
  image.setAttribute("src", currentUser.picture.large);
}
